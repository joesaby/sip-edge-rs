// transport.rs - Async network transport with TLS support

use crate::{
    codec::SipCodec,
    security::{SecurityConfig, SecurityValidator},
    SipMessage, SipParseError,
};
...
impl SipTransport {
                        ...
                        match socket.recv_from(&mut buf).await {
                            Ok((len, peer_addr)) => {
                                ...
                                match crate::parse_sip_message(data) {
                                    Ok(message) => {
                                        // Security validation
                                        if let Err(e) = security_validator.validate_message(&message, &peer_addr.to_string()) {
                                    ...
                                    Err(e) => {
                                        warn!("Failed to parse UDP message from {}: {}", peer_addr, e);
                                        let _ = event_tx.send(TransportEvent::Error {
    ...
    fn start_tcp_listener(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            match TcpListener::bind(addr).await {
                ...
                Err(e) => {
                    error!("Failed to bind TCP socket on {}: {}", addr, e);
                }
            }
        })
    }
    ...
    async fn handle_tls_connection(
        ...
        Self::handle_stream_connection(stream, peer_addr, event_tx, security_validator, TransportProtocol::Tls).await;
    }
    ...
    pub async fn send_message(
        ...
        match transport {
            TransportProtocol::Udp => {
                self.send_udp_message(message, destination).await
            }
            TransportProtocol::Tcp => {
                self.send_tcp_message(message, destination).await
            }
            TransportProtocol::Tls => {
                self.send_tls_message(message, destination).await
            }
        }
    }
    ...
    async fn send_tls_message(&self, message: SipMessage, destination: SocketAddr) -> Result<()> {
        ...
        let domain = rustls::ServerName::try_from("sip.example.com")
            .map_err(|_| anyhow::anyhow!("Invalid domain name"))?;
