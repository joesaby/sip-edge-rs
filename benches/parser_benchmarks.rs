...
fn benchmark_security_validation(c: &mut Criterion) {
    ...
    c.bench_function("security_validation", |b| {
        b.iter(|| {
            let _result = validator.validate_message(black_box(&message), "127.0.0.1");
        })
    });
}
...
fn benchmark_codec_encode(c: &mut Criterion) {
    ...
    headers.insert(
        "via".to_string(),
        vec!["SIP/2.0/UDP pc33.atlanta.com".to_string()],
    );
    headers.insert(
        "from".to_string(),
        vec!["<sip:alice@atlanta.com>;tag=123".to_string()],
    );
    headers.insert("to".to_string(), vec!["<sip:bob@biloxi.com>".to_string()]);
    headers.insert(
        "call-id".to_string(),
        vec!["test123@atlanta.com".to_string()],
    );
